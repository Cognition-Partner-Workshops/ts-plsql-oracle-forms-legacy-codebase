...
BEGIN
    ...
    IF :OLD.EMPLOYMENT_STATUS != :NEW.EMPLOYMENT_STATUS THEN
        INSERT INTO EMPLOYEE_HISTORY (
            ...
            'STATUS_UPDATE', :OLD.EMPLOYMENT_STATUS || ' -> ' || :NEW.EMPLOYMENT_STATUS,
            NVL(:NEW.MODIFIED_BY, USER), SYSDATE
        );
    END IF;
    ...
    IF NVL(:OLD.DEPT_ID, -1) != NVL(:NEW.DEPT_ID, -1) THEN
        INSERT INTO EMPLOYEE_HISTORY (
            HIST_ID, EMP_ID, CHANGE_TYPE, EFFECTIVE_DATE,
            OLD_DEPT_ID, NEW_DEPT_ID, REASON_CODE, CREATED_BY, CREATED_DATE
        ) VALUES (
            SEQ_EMP_HISTORY.NEXTVAL, :NEW.EMP_ID, 'DEPARTMENT_CHANGE', SYSDATE,
            :OLD.DEPT_ID, :NEW.DEPT_ID,
            'DEPT_TRANSFER', NVL(:NEW.MODIFIED_BY, USER), SYSDATE
        );
    END IF;
    ...
    IF NVL(:OLD.JOB_ID, -1) != NVL(:NEW.JOB_ID, -1) THEN
        INSERT INTO EMPLOYEE_HISTORY (
            HIST_ID, EMP_ID, CHANGE_TYPE, EFFECTIVE_DATE,
            OLD_JOB_ID, NEW_JOB_ID, REASON_CODE, CREATED_BY, CREATED_DATE
        ) VALUES (
            SEQ_EMP_HISTORY.NEXTVAL, :NEW.EMP_ID, 'JOB_CHANGE', SYSDATE,
            :OLD.JOB_ID, :NEW.JOB_ID,
            'JOB_CHANGE', NVL(:NEW.MODIFIED_BY, USER), SYSDATE
        );
    END IF;
